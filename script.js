...
function getUnit(desc) {
  ...
  if (d.includes('cast iron deluge valve')) return 'Nos';
  if (d.includes('m.s pipes heavy c class')) return 'Meter';
  if (d.includes('g.i. pipes heavy c class')) return 'Meter';
  if (d.includes('wafer type butterfly valve')) return 'Nos';
  if (d.includes('y type strainers')) return 'Nos';
  if (d.includes('medium velocity water spray nozzle')) return 'Nos';
  if (d.includes('digital linear heat detection cable')) return 'Nos';
  if (d.includes('deluge valve control panel outdoor')) return 'Nos';
  if (d.includes('pressure switch')) return 'Nos';
  if (d.includes('24 vdc solenoid valve')) return 'Nos';
  if (d.includes('monitor module')) return 'Nos';
  if (d.includes('control nodule') || d.includes('control module')) return 'Nos';
  if (d.includes('12v') && d.includes('10 amps battery')) return 'Nos';
  if (d.includes('cables and accessories')) return 'Lot';
  if (d.includes('hardware like nut bolts') || d.includes('green gasket')) return 'Lot';
  if (d.includes('ms support made of l angle') || d.includes('ms plate etc')) return 'Kg';
  return '';
}
...
function renderBOM(st) {
  ...
  for (let r = 29; r <= 47; r++) {
    ...
    const tdu = document.createElement('td'); tdu.textContent = getUnit(desc);
    const tdd = document.createElement('td'); tdd.textContent = desc;
    tr.appendChild(tdq); tr.appendChild(tdu); tr.appendChild(tdd);
